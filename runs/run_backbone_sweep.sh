...
# Script to compare different single-view transformer backbones on ibl-mouse dataset
# Runs each backbone with 3 seeds (0, 1, 2) and saves results to Lightning Storage
#
# Usage:
#   ./run_backbone_sweep.sh                          # Run all backbones on CUDA device 0
#   ./run_backbone_sweep.sh 1                        # Run all backbones on CUDA device 1
#   ./run_backbone_sweep.sh 0 --backbones vitb_dino,vits_dinov2   # Run specific backbones
#   ./run_backbone_sweep.sh 0 --seeds 0,1            # Run specific seeds only
...
PIPELINE_CONFIG="${CONFIG_DIR}/pipeline_inference_backbone_sweep_ibl-mouse.yaml"
LIGHTNING_CONFIG="${CONFIG_DIR}/config_ibl-mouse_backbone_sweep.yaml"
...
echo "=========================================="
echo "Backbone Sweep - ibl-mouse (Single View)"
echo "=========================================="
...
for backbone in "${BACKBONES[@]}"; do
    COUNTER=$((COUNTER + 1))
    RESULTS_NAME="test_200_SV_${backbone}"
...
echo "  Subdirectories:"
for backbone in "${BACKBONES[@]}"; do
    echo "    test_200_SV_${backbone}/"
    for s in "${SEEDS[@]}"; do
        echo "      supervised_200_${s}/"
    done
done
echo ""
...
# ./lp3d-analysis/runs/run_backbone_sweep.sh 0